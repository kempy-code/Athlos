...
function goalCard(goal) { const overdue = goal.targetDate && new Date(goal.targetDate) < new Date() && goal.status !== "completed"; return `<article class="tool-row"><div><strong>${escapeHtml(goal.title)}</strong><span>Target ${escapeHtml(goal.target)} · ${escapeHtml(goal.targetDate)}</span></div>${goal.status === "completed" ? '<b class="complete-chip">Complete</b>' : `<button data-goal-status="${goal.id}" class="tiny-button" type="button">${overdue ? "Review" : "Mark complete"}</button>`}</article>`; }
